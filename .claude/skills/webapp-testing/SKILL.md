...
## Скриншот перед завершением (обязательно)

Любая UI-задача (включая использование этого скилла) считается невыполненной без финального скриншота и сверки с требованиями.

Чек-лист перед заявлением «готово»:

1. `page.screenshot(path='final.png', full_page=True)` после всех действий
2. Сверить с требованиями: расположение, цвет, текст, состояния
3. Если headless среда не даёт визуально проверить — маркер «⚠️ визуально не проверил, причина: ...» в финальном ответе

Hook `screenshot-gate.py` (Stop) блокирует завершение при отсутствии скриншота или маркера. Источник правила: `memory/feedback_screenshot_before_done.md`, calibration_log 2026-05-04.
